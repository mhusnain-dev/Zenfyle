...
- **zod caveat:** `app/api/jobs/route.ts` imports `zod` (v4.4.3) but zod is **not yet a declared dependency** in package.json (only transitively present via another package, so build passes today). Add `zod` to `dependencies` explicitly before relying on it further — it's a latent break if the transitive provider changes.

### Server-tool binary invocation rule
Every external binary (gs, soffice, qpdf, …) is invoked with `spawn` from **exactly one adapter file** under `lib/server/tools/` (the server mirror of the §585 adapter pattern — the spec's prose says `server/workers/` but the actual, agreed location is `lib/server/tools/`). Adding a server tool = one adapter file + one line in `lib/server/tools/index.ts`. Pass an `AbortSignal` to `spawn` for cancellation; throw `ProcessingError(userMessage)` (from `lib/server/tools/types.ts`) for input problems so the pipeline maps them to a user-facing message instead of `UNKNOWN_ERROR`.
