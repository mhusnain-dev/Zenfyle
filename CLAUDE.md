...
**`specs.md` (v1.4.4, frozen) is the single source of truth. Code is a build output. Read it before writing any code — especially Section 11 (architecture blueprint) and Section 9 (phase order).**
...
> Spec version note: `specs.md` §changelog is at **v1.4.4** (v1.0.0 was the frozen baseline; v1.1.0–v1.4.0 were owner-requested visual/icon refinements; v1.4.1–v1.4.2 added implementation infrastructure agreed during Phases 6–7, and v1.4.3–v1.4.4 the OCR-backed tools + the OCR category, agreed during/after Phase 8). The core architecture/phases/registry contract is unchanged from v1.0.0, except the additive sixth `ocr` category (v1.4.4).
...
- **DB:** Postgres everywhere (schema provider = `postgresql`, `@prisma/adapter-pg` in `lib/db.ts`). Local dev points `DATABASE_URL` at the same Supabase/Neon Postgres as prod. Prisma 7 rejects a driver adapter whose dialect differs from the schema provider, so there is NO SQLite path — do not try to reintroduce one. Models use `String` for enum-like fields (`status`, `optionsJson`) — keep them string on Postgres.
...
- **Installed & confirmed working:** Ghostscript `gs` 10.06.0 (`/usr/bin/gs`), LibreOffice `soffice`/`libreoffice` 26.2.4.2 (`/usr/bin`), qpdf 12.3.2 (user-space, see above), Tesseract 5.5.0 (user-space — same .deb trick, extracted to `~/.local/tesseract` with a `~/.local/bin/tesseract` wrapper that sets `LD_LIBRARY_PATH` + `TESSDATA_PREFIX`; used by Redact PDF and PDF→Excel via the single `lib/server/tools/ocr.ts` adapter). Production must install `tesseract-ocr` + `eng` traineddata as a real package.
- **NOT installed:** Redis (fine — local uses the in-process queue). If a future phase needs a new system package, use the same user-space .deb trick or mark the tool `comingSoon`.
...
- `app/api/jobs/route.ts` — `POST`: validate request → gate tool (client-only / comingSoon / unimplemented → `TOOL_UNAVAILABLE`) → content-validate upload (`lib/server/validate-upload.ts`, magic bytes / zero-byte / size, server-side, never trust extension) → identify requester (`auth()` session → `userId`, else salted `ipHash`) → concurrency guard (one active `queued`/`processing` job per identity → `QUEUE_FULL`) → rate-limit check (`getRateLimiter().check`, 20/day anon by ipHash, 50/day user by userId → `RATE_LIMIT_EXCEEDED`) → content-validate upload (`lib/server/validate-upload.ts`, magic bytes / zero-byte / size, server-side, never trust extension) → create `Job` row (with `userId`/`ipHash`) → store input → record `usageEvent` (the rate-limit increment; IP is salted-hashed, `IP_HASH_SALT`) → `await getQueue()` then enqueue.
...
**Error handling convention:** every API error is `{ error: { code, message } }` via `apiError(code, msg)` (`lib/server/api-error.ts`) using the fixed §13.7 enum (`FILE_TOO_LARGE`, `UNSUPPORTED_FILE_TYPE`, `FILE_CORRUPTED`, `FILE_ENCRYPTED`, `INVALID_PASSWORD`, `TOOL_UNAVAILABLE`, `QUEUE_FULL`, `QUEUE_TIMEOUT`, `WORKER_ERROR`, `RATE_LIMIT_EXCEEDED`, `UNKNOWN_ERROR`). Never leak stack traces/library text — log the real error server-side, return the enum code. The worker→client error path carries both `errorMessage` (string) and `errorCode` (§13.7 enum) on the job row (v1.4.1); the GET poll surfaces `error_code` and the client branches on it (e.g. `INVALID_PASSWORD` → the calm "that password didn't work" banner).

**Job schema** (`prisma/schema.prisma`, `Job` model, all snake_case `@map`): `id` (cuid), `userId?` (nullable — anonymous allowed), `toolSlug`, `status` (queued|processing|success|error|cancelled|expired), `originalFilename`, `mimeType`, `fileSizeBytes`, `optionsJson?` (text, JSON-serialized options — password is stripped out first, v1.4.1), `errorMessage?` (also reused for the success "note"), `errorCode?` (§13.7 code, v1.4.1), `resultSummary?` (success outcome line shown on the result screen — **schema added but migration not yet run + read-path half-wired; see PROGRESS.md "🚧 In progress"**), `ipHash?` (v1.4.2 — scopes the anon concurrency guard), `inputFileRef?`, `outputFileRef?` (@unique — doubles as the download-URL lookup), `outputFileSizeBytes?`, `outputFileCount`, `progressStage?`, `progressPercent`, timestamps (`createdAt`/`startedAt`/`completedAt`/`expiresAt`). `UsageEvent` model backs rate limiting (`userId?`, `ipHash`, `toolSlug`, timestamp). `User` (email, `passwordHash`, `planTier`) + `PasswordResetToken` (v1.4.2 — `userId`, `tokenHash` @unique, `expiresAt`; only a SHA-256 hash of the token is stored) back Phase 7 auth.

**Auth + rate limiting (Phase 7, v1.4.2):** Auth.js 5 with the **JWT session strategy** (no DB session table) + a Credentials provider (email/password, bcryptjs). Split config so the Edge middleware never loads Node deps: `auth.config.ts` (edge-safe — session/pages/callbacks, empty providers) is used by `proxy.ts` (Next 16 renamed `middleware.ts` → `proxy.ts`) to gate `/dashboard`; `auth.ts` spreads it and adds the Credentials provider (Prisma + bcryptjs, Node runtime). Auth write-paths (signup, request-reset, reset, change-password, delete-account) are **Server Actions** in `lib/server/auth-actions.ts`, not API routes; login/logout use Auth.js's own handlers. The Header reads session via `useSession` (client) behind a `SessionProvider` in the root layout — **never call `auth()` in the root layout** (it forces every page dynamic and kills the homepage/tool-page static+SSG rendering). Two more env-swap interfaces mirror storage/queue: `MailProvider` (`lib/server/mail/`, `MAIL_PROVIDER` — `console` logs the reset link locally, `smtp` sends via nodemailer) and `RateLimiter` (`lib/server/rate-limit/`, `RATE_LIMITER` — `db` counts `usage_events` since UTC midnight, `redis` is the prod counter). Rate limits (§13.4): anon 20/day by `ipHash`, logged-in 50/day by `userId`; plus a one-active-job concurrency guard (`QUEUE_FULL`). Reset tokens: only a SHA-256 hash is stored, 1h expiry, single-use.

## Conventions specific to this project
- **`.gitignore`:** `.storage/` is gitignored — don't commit it.
