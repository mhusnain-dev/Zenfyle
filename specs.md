...
**v1.4.4 — OCR category + OCR PDF tool (owner-requested, post-Phase 8).** The owner asked for OCR to have a visible home in the product: a header tab adjacent to Security. A nav tab is category-driven (Section 3/4), so this adds a sixth tool category **`ocr`** (label "OCR", ordered after `security` in `CATEGORY_ORDER`) and one tool behind it: **OCR PDF** (`ocr-pdf`, server, active). The tool makes a scanned/image-only PDF searchable: each page is rasterized (Ghostscript, 300 DPI via the v1.4.3 `ocr.ts` adapter), OCR'd (Tesseract), and rebuilt with pdf-lib as the flattened page image plus an **invisible** text layer (`setTextRenderingMode(Invisible)`) carrying the recognized words at their positions — the visible page is unchanged, but its text becomes searchable/selectable. This is exactly redact-pdf's flatten + re-OCR pipeline minus the region blackout, so no new dependency or spawn point. Honesty rules (§592): if OCR recognizes zero words document-wide the job fails with `UNSUPPORTED_FILE_TYPE` ("nothing to make searchable") instead of returning a pointlessly rasterized copy, and every success carries a note that recognition is best-effort and the tool is meant for scans (an already-digital PDF would only lose crisp vector text by being flattened). Registry additions: category accent tokens (`--color-cat-ocr*`, sky-blue hue, AA-checked like the v1.3.0 set), Lucide `scan-text` icon registered in `lib/icons.ts`, `NoOptions` panel (no settings). Limits: 50 MB (the "everything else" tier, §13.2). Verified end-to-end over real HTTP: image-only fixture (0 extractable chars) → output extracts the full fixture text; header renders the OCR tab after Security on desktop and in the mobile drawer. No routing, folder-layout, or phase-order changes beyond the additive category.
**v1.4.3 — OCR-backed Redact PDF + PDF→Excel; processing-matrix correction (during Phase 8, Convert & advanced tools).** Implementing the last two `comingSoon` tools surfaced a factual error in the frozen spec and the concrete approach for both, agreed with the owner before coding: **(1) Processing-matrix correction.** The matrix (Section 11.5) listed **PDF to Excel** as "LibreOffice headless" — but LibreOffice has **no PDF→Calc import filter** (it opens PDFs only in Draw), so that row was never implementable as written and the tool correctly sat `comingSoon` under the anti-hallucination rule (§592). Corrected to describe the real approach: **geometry-based table reconstruction + Tesseract OCR fallback.** The digital path pulls positioned words from the PDF text layer (pdfjs, via `extractPositionedText` added to the single `pdf-text.ts` pdfjs adapter); pages with no text layer (scanned/image) fall back to OCR. Words are clustered into rows (by vertical center) and columns (by x-axis gap clustering) and written one worksheet per page with **`exceljs`** (new pinned dependency, `exceljs@4.4.0`). Table extraction is inherently best-effort, so every result carries an explicit "check the columns/rows" note, and a PDF that yields nothing extractable fails with `UNSUPPORTED_FILE_TYPE` rather than emitting an empty sheet (§592 "when uncertain, fail explicitly"). **(2) Redact PDF is OCR-backed (§4.1c permanent removal).** To guarantee removal from *both* pixels and the text layer, each page is flattened: rasterized (Ghostscript, 300 DPI), the marked regions burned to solid black on the pixels (`sharp`), then the already-redacted image is re-OCR'd (Tesseract) to rebuild an **invisible** searchable text layer (pdf-lib `setTextRenderingMode(Invisible)`) — the blacked-out areas contain no legible pixels, so the removed text cannot reappear in the text layer. Selection UI (`RedactOptions`) captures boxes in normalized page coords via pdf.js, mirroring the Annotate editor. Verified end-to-end: a redacted SSN line is absent from the output's extractable text while the rest of the document stays searchable. **(3) OCR adapter.** All Tesseract use sits behind one new `lib/server/tools/ocr.ts` adapter (the single `tesseract` spawn point, mirroring the qpdf adapter rule, §11.1/585); no secrets flow through it so args go on argv. On the dev box Tesseract is a **user-space install** (same `dpkg-deb -x` extraction + `~/.local/bin` wrapper as qpdf), so production must install `tesseract-ocr` (with `eng` traineddata) as a real package — same deployment note that already applies to qpdf/Ghostscript/LibreOffice. No routing, folder-layout, or phase-order changes.
**v1.4.2 — SMTP mail provider + Phase 7 auth/account infrastructure (during Phase 7, Accounts & Rate Limiting).** Building Phase 7 surfaced concrete additions the frozen spec named only at the interface level, all agreed with the owner before/during coding: **(1) SMTP MailProvider.** Alongside the local-dev `ConsoleMailProvider` (logs the reset link to the server console — no SMTP in the sandbox), added `SmtpMailProvider` (`lib/server/mail/smtp.ts`), a `nodemailer`-backed implementation of the same `MailProvider` interface, selected by `MAIL_PROVIDER=smtp`. Config via `SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASS` + `MAIL_FROM`; port 465 ⇒ implicit TLS, else STARTTLS. `nodemailer` (+ `@types/nodemailer`) promoted to a direct dependency. For the owner's testing this is wired to Gmail SMTP with an App Password (credentials live only in gitignored `.env`); Gmail forces `From` to equal the authenticated account and throttles ~500/day, so it's a **test-only** sender — production swaps to a transactional provider (Resend/Postmark/SES) behind the identical interface with no caller changes. Delivery was verified live (`transporter.verify()` + a `250 OK` send). **(2) `PasswordResetToken` model** — one-time reset tokens stored as a sha256 hash (never plaintext), 1-hour expiry, consumed on use; the reset link embeds the raw token. **(3) `Job.ipHash`** — anonymous jobs/usage are keyed by a salted sha256 of the client IP (`IP_HASH_SALT`, truncated), never the raw IP, so the DB rate limiter and per-IP concurrency guard work without storing PII. **(4) DB RateLimiter** (`RATE_LIMITER=db`, default) — counts `usage_events` since the start of the UTC day: 20/day anon (keyed by `ipHash`), 50/day logged-in (keyed by `userId`), returning `RATE_LIMIT_EXCEEDED` (HTTP 429) over the cap, plus a one-active-job concurrency guard returning `QUEUE_FULL`; `RATE_LIMITER=redis` (Upstash) slots in behind the same interface for production. All four verified with the owner: real reset email delivery, and the 429 blocking the over-limit request then releasing when back under. No routing, folder-layout, or phase-order changes.
...
**OCR (v1.4.4):** OCR PDF (make a scanned PDF searchable)
...
| PDF to Excel | Server | Table reconstruction from word geometry (pdfjs text layer) + Tesseract OCR fallback for scanned pages; exceljs writer — best-effort, NOT LibreOffice (no PDF→Calc filter exists). See Changelog v1.4.3. |
...
| Redact PDF | Server | permanent removal, not an overlay — see scope in 4.1c. Flatten each page (Ghostscript raster → burn regions black with sharp) then rebuild an invisible searchable text layer via Tesseract OCR (pdf-lib). Removed content is gone from both pixels and text. See Changelog v1.4.3. |
| Compare PDF | Server | text-diff across two documents (scope limited per 4.1c) — heavier computation |
| OCR PDF | Server | make a scanned PDF searchable: rasterize each page (Ghostscript) → Tesseract OCR → rebuild flattened image + invisible text layer (pdf-lib). Reuses redact-pdf's pipeline minus the blackout. See Changelog v1.4.4. |
